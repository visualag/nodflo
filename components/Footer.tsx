...
const SOCIAL_ICONS: Record<string, React.ReactElement> = {
    ...
    x: (
        <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.73-8.835L1.254 2.25H8.08l4.26 5.632L18.243 2.25zm-1.161 17.52h1.833L7.084 4.126H5.117z" /></svg>
    ),
};
...
export default function Footer() {
    ...
    const columns = settings?.footerColumns?.length ? settings.footerColumns : DEFAULT_FOOTER_COLUMNS;
    const social = settings?.socialLinks || {};

    const activeSocial = Object.entries(social).filter(([, url]) => !!url) as [string, string][];
        ...
        <footer className="footer">
            <div className="container">
                <div className="footer__grid">
                    <div>
                        <div className="footer__logo">
                            {logoUrl
                                ? <img src={logoUrl} alt={name} style={{ height: 28, objectFit: "contain", filter: "brightness(0) invert(1)", opacity: 0.9 }} />
                                : name
                            }
                        </div>
                        <p className="footer__desc">{desc}</p>
                        <p style={{ fontSize: "0.8rem", color: "rgba(245,244,240,0.4)" }}>{addr}</p>
                        ...
                        <div key={i}>
                            ...
                            <div className="footer__links">
                                {col.links?.map((l: any, j: number) => (
                                    <Link key={j} href={l.href} className="footer__link">{l.label}</Link>
                                ))}
                            </div>
                        </div>
                    ))}
                ...
                <div className="footer__bottom">
                    <span>{copyright}</span>
                    <span>
                        <Link href="/admin" style={{ color: "rgba(245,244,240,0.2)" }}>Admin</Link>
                    </span>
                </div>
            </div>
        </footer>
